<div class="alert alert-info">当前位置<b class="tip"></b>公司设置<b class="tip"></b>董事长专栏
<php>
    if($show_list == 0)
    {
...
<php>
	for($i = 0 ; $i < count($managers); $i++ )
	{
            ...
            <tr class="tr_chairman">
                <td class="my_td td_odrer">{$i+1}</td>
                <td>{$managers[$i].title}</td>
                <td>{$managers[$i].time}</td>
                <td>
                	<a href="/Cms/{$CURRETN_CONTROLLER}/{$CURRETN_MODULE}/{$managers[$i][id]}"><button class="btn btn-success edit" data-toggle="tooltip" title="编辑"><span class="glyphicon glyphicon-edit"></span></button></a>
<!--                     <button class="btn btn-success up move" data-toggle="tooltip" title="上移" url-link ="/Cms/Common/move/managers/up/{$managers[$i][id]}"><span class="glyphicon glyphicon-arrow-up"></button>
                    <button class="btn btn-success down move" data-toggle="tooltip" title="下移" url-link ="/Cms/Common/move/managers/down/{$managers[$i][id]}"><span class="glyphicon glyphicon-arrow-down"></button> -->
                    <button class="btn btn-danger del" data-toggle="tooltip" title="删除" url-link ="/Cms/Common/delete/managers/0/{$managers[$i][id]}"><span class="glyphicon glyphicon-trash"></span></button>
                </td>
            </tr>
<php>
	}
...
<php>
	if( $show_list == 0 )
	{
		...
		<div class="manager-avatar">
			<img class="img-avatar" src="{$managers[0].img_url}" alt="图片加载失败">
        </div>
		...
		<div class="col-md-5">
        	...
        	<div class="one-line">
        	    <div class="input-group">
        	        ...
        	        <input id = "m_title" type="text" class="form-control" aria-describedby="sizing-addon1" value="{$managers[0].title}">
        	    </div>
        	</div>
        	...
        	<div class="one-line">   
        	    <div class="input-group">
                  ...
                  <input id = "m_time" type="text" class="form-control" aria-describedby="sizing-addon1" value="{$managers[0].time}" readOnly>
                </div>
        	</div>
        </div>
        ...
        <div class="col-md-12 one-line">                        
            <div class="input-group">
                ...
                <textarea id = "m_content" class="form-control" rows="8">{$managers[0].content}</textarea>
            </div>     
        </div>
